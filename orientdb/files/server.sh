...
MAXHEAP=-Xmx{{environment.java_max_heap}}
# ORIENTDB MAXIMUM DISKCACHE IN MB, EXAMPLE, ENTER -Dstorage.diskCache.bufferSize=8192 FOR 8GB
MAXDISKCACHE=""

exec "$JAVA" $JAVA_OPTS $MAXHEAP $JAVA_OPTS_SCRIPT $ORIENTDB_SETTINGS $MAXDISKCACHE -Djava.util.logging.config.file="$LOG_FILE" -Dorientdb.config.file="$CONFIG_FILE" -Dorientdb.www.path="$WWW_PATH" -Dorientdb.build.number="UNKNOWN@r${buildNumber}; 2015-07-09 15:36:50+0000" -cp "$ORIENTDB_HOME/lib/orientdb-server-2.2-SNAPSHOT.jar:$ORIENTDB_HOME/lib/*" $* com.orientechnologies.orient.server.OServerMain
